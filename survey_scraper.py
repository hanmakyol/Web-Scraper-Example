...
if user_input == 'y':
    if data:
        ...
        create_table_query = f"CREATE TABLE IF NOT EXISTS survey_of_british_lit ({columns_string})"
        db_cursor.execute(create_table_query)
        db_connection.commit()

        for row in data:
            placeholders = ", ".join(["%s"] * len(row))
            sql_insert = f"INSERT INTO survey_of_british_lit VALUES ({placeholders})"
