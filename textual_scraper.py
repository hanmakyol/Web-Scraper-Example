...
url = 'http://bbs.ankara.edu.tr/Ders_Bilgileri.aspx?dno=1008747&bno=1596&bot=193'
...
if user_input == 'y':
    if data:
        ...
        create_table_query = f"CREATE TABLE IF NOT EXISTS textual_analysis ({columns_string})"
        db_cursor.execute(create_table_query)
        db_connection.commit()

        for row in data:
            placeholders = ", ".join(["%s"] * len(row))
            sql_insert = f"INSERT INTO textual_analysis VALUES ({placeholders})"
